...
namespace DAL
{
    public class ContractApproval_DAL
    {
        ...
        {
            string formID = "";
            string sql = "select Form_ID from As_Contract_Approval where Temp_Vendor_ID=@Temp_Vendor_ID and Form_Type_ID=@Form_Type_ID and Factory_Name=@Factory_Name and ID=@ID";
            SqlParameter[] sp = new SqlParameter[]
            {
        ...
        public static string getFileTypeName(double money, string promise)
        {
            ...
            {
                sql = "select File_Type_Name from As_File_Type where File_Type_Name like '合同审批表_承诺%' and (File_Type_Name not like '合同审批表_非承诺%') and Min_Money<@money and Max_Money>@money";
            }
            else
            {
                sql = "select File_Type_Name from As_File_Type where File_Type_Name like '合同审批表_非承诺%' and Min_Money<@money and Max_Money>@money";
            }
        ...
        {
            string sql = "insert into As_Contract_Approval(Temp_Vendor_ID,Vendor_Name,Form_Type_ID,Flag,Factory_Name) values(@Temp_Vendor_ID,@Vendor_Name,@Form_Type_ID,@Flag,@Factory_Name) select TOP 1 SCOPE_IDENTITY() AS returnName from As_Contract_Approval";
            SqlParameter[] sp = new SqlParameter[]
            {
